...
const CreateTemplate = () => {
  ...
  //       timer: 10000, // set the alert showing time 
  //     });
  //   } else {
  //     Swal.fire({
  //       icon: "success",
  ...
  const handleButton = () => {
    if (!templateName || !subjectName || !data) {
      Swal.fire({
        ...
        title: "Oops...",
        text: "Please fill out all fields!",
        allowOutsideClick: false, // prevent the user from clicking outside the alert box
        allowEscapeKey: false, // prevent the user from dismissing the alert using the Escape key
        timer: 2000, // set the alert showing time
      });
    } else {
      Swal.fire({
        icon: "success",
